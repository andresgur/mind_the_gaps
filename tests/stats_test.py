...
class TestSimulator(unittest.TestCase):
    ...
    def test_lognormal(self):
        ...
        self.assertAlmostEqual(
            np.mean(samples), np.mean(samples_2), delta=0.5,
            msg="Log normal probability distribution mean fails"
        )
        self.assertAlmostEqual(
            np.std(samples), np.std(samples_2), delta=0.5,
    ...
    def test_chi_loglikelihood(self):
        ...
        self.assertAlmostEqual(
            log_like, chi_log_likehood(data_powers, model_powers, False),
            msg="Log-likehood gives unexpected results when Nyquist is not included", delta=1E-12
        )
        ...
        self.assertEqual(
            log_like, chi_log_likehood(data_powers, model_powers, True),
