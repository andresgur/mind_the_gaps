...
class TestSimulator(unittest.TestCase):
    ...
    def test_mean(self):
        ...
        means = [0, 100]
        for mean in means:
            simu = Simulator(psd_model, times, exposures, mean, "Gaussian", extension_factor=1.05, aliasing_factor=2)
            means = []
            for i in range(30):
                lc = simu.simulate_regularly_sampled()
    ...
    def test_pdf_lognormal(self):
        inputmean = 10
        inputvariance = 10
        simu = self.prepare_simulator("Lognormal", inputmean, inputvariance)
        lc = simu.simulate_regularly_sampled()
        segment = cut_random_segment(lc, simu.sim_duration)
        sample_std = np.std(segment.countrate)
        pdf = simu.simulator.pdfmethod(inputmean, sample_std)
        E13_lc = simu.simulator.adjust_lightcurve_pdf(segment, pdf, max_iter=simu.simulator.max_iter)
        var = sample_std**2
        inputmu = np.log((inputmean**2) / np.sqrt(var + inputmean**2))
        inputsigma = np.sqrt(np.log(var/(inputmean**2) + 1))
        fitresult = fit(lognorm, E13_lc.countrate,  
    ...
    def test_pdf_uniform(self):
        inputmean = 10
        inputvariance = 10
        simu = self.prepare_simulator("Uniform", inputmean, inputvariance)
        ...
        E13_lc = simu.simulator.adjust_lightcurve_pdf(segment, pdf, max_iter=simu.simulator.max_iter)
        var = sample_std**2
        inputb = np.sqrt(3 * var) + inputmean
        inputa = 2 * inputmean - inputb
        fitresult = fit(uniform, E13_lc.countrate,  
