...
Two folders in [tutorials](docs/notebooks/) exist, Celerite and Celerite2. The code has two engines which can be used to test for periodicity in a lightcurve, a [celerite](https://celerite.readthedocs.io/en/stable/) based engine which uses [emcee](https://emcee.readthedocs.io/en/stable/) to perform the sampling in order to derive the posteriors, and its JAX enabled relative [celerite2](https://celerite2.readthedocs.io/en/latest/api/python/) which uses [numpyro](https://num.pyro.ai/en/latest/index.html) for sampling. 

A class exists to perform the workflow in order to detect a periodicity, the use of which can be seen in [celerite comparison tutorial](docs/notebooks/celerite/tutorial_ppp_comp.ipynb) and [celerite comparison tutorial](docs/notebooks/celerite2/tutorial_ppp_celerite2_comp.ipynb) 
