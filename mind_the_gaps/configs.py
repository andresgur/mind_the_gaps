...
from mind_the_gaps.models.celerite.celerite_models import (
    BendingPowerlaw,
    Cosinus,
    DampedRandomWalk,
    Lorentzian,
...
def read_config_file(config_file, walkers=32):
    ...
    try:
        ...
        for kernel_counter, row in enumerate(model_info):
            ...
            w = np.log(
                two_pi / (np.array(row["P"].split(":")).astype(float) * days_to_seconds)
            )
            S_0 = np.array(row["logS_0"].split(":")).astype(float)  # already in log

            if (
                row["model"] == "Lorentzian"
                or row["model"] == "SHO"
                or row["model"] == "Powerlaw"
            ):
                Q = np.log(np.array(row["Q"].split(":")).astype(float))
                bounds = dict(
                    log_S0=(S_0[0], S_0[2]), log_Q=(Q[0], Q[2]), log_omega0=(w[2], w[0])
                )

                # create the variables if it's the first kernel
                kernel_columns = [
                    "kernel:%slog_omega0" % kernel_string,
                    "kernel:%slog_S0" % kernel_string,
                    "kernel:%slog_Q" % kernel_string,
                ]
                kernel_labels = [r"$P$ (days)", r"log $S_0$", r"log Q"]
                init_model_pars = np.array(
                    [
                        np.random.uniform(S_0[0], S_0[2], walkers),
                ...
                if row["model"] == "SHO":
                    kernel = celerite.terms.SHOTerm(
                        log_S0=S_0[1], log_Q=Q[1], log_omega0=w[1], bounds=bounds
                    )
                elif row["model"] == "Lorentzian":
                    kernel = Lorentzian(
                        log_S0=S_0[1], log_omega0=w[1], log_Q=Q[1], bounds=bounds
                    )
                elif row["model"] == "Powerlaw":
                    kernel = BendingPowerlaw(
                        log_S0=S_0[1], log_omega0=w[1], log_Q=Q[1], bounds=bounds
                    )

            # two param model components (S_0 and omega)
            elif (
                row["model"] == "DampedRandomWalk"
                or row["model"] == "Granulation"
                or row["model"] == "Cosinus"
            ):
                bounds = dict(log_S0=(S_0[0], S_0[2]), log_omega0=(w[2], w[0]))

                if row["model"] == "DampedRandomWalk":
                    kernel = DampedRandomWalk(
                        log_S0=S_0[1], log_omega0=w[1], bounds=bounds
                    )
                elif row["model"] == "Granulation":
                    Q = 1 / np.sqrt(2)
                    kernel = celerite.terms.SHOTerm(
                        log_S0=S_0[1], log_Q=np.log(Q), log_omega0=w[1], bounds=bounds
                    )
                ...
                kernel_columns = [
                    "kernel:%s%s" % (kernel_string, name)
                    for name in kernel.get_parameter_names()
                ]
                ...
                log_rho = np.log(
                    np.array(row["P"].split(":")).astype(float) * days_to_seconds
                )
                bounds = dict(
                    log_sigma=(S_0[0], S_0[2]), log_rho=(log_rho[0], log_rho[2])
                )
                kernel = celerite.terms.Matern32Term(
                    log_sigma=S_0[1], log_rho=log_rho[1], eps=1e-7, bounds=bounds
                )
