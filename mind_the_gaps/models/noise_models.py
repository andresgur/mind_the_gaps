...
class PoissonNoise(BaseNoise):
    ...
    def add_noise(self, rates):
        ...
        dy = np.sqrt(
            (np.sqrt(total_counts_poiss) / self.exposures) ** 2 + self.bkg_rate_err**2
        )
...
class KraftNoise(PoissonNoise):
    ...
    def add_noise(self, rates):
        ...
        if np.any(expression):
            ...
            net_counts[expression] = [
                pdf(counts, bkg_counts_).median()
                for counts, bkg_counts_ in zip(
                    np.round(total_counts[expression]).astype(int),
                    self.background_counts[expression],
                )
            ]
            ...
            lower_confs, upper_confs = poisson_conf_interval(
                total_counts[expression].astype(int),
                "kraft-burrows-nousek",
                background=self.background_counts[expression],
                confidence_level=0.68,
            )
            dy[expression] = (
                (upper_confs - lower_confs) / 2 / self.exposures[expression]
            )  # bayesian
