...
def read_data(input_file, tmin=0, tmax=np.inf):
    ...
    filtered_data = data[
        np.where(
            (data["%s" % time_column] >= tmin) & (data["%s" % time_column] <= tmax)
        )
    ]
    if time_column == "MJD":
    ...
    yerr = (
        -filtered_data["%sneg" % rate_column] + filtered_data["%spos" % rate_column]
    ) / 2
    ...
    return (
        ...
        y,
        yerr,
        exposures / corr_factor,
        bkg_counts,
        bkg_rate_err * corr_factor,
    )
...
def readPCCURVE(file="PCCURVE.qdp", minExposure=0, minSigma=0, minSNR=0, minCts=0):
    ...
    filtered_data = data[
        (data["Exposure"] > minExposure)
        & (data["SNR"] > minSNR)
        & (data["Sigma"] > minSigma)
        & (data["CtsInSrc"] >= minCts)
    ]
    filtered_obs = len(data) - len(filtered_data)
    print(
        "Filtered %d datapoints by minSNR = %d and minSigma = %.2f, minCts = %d and minExp=%.1f"
        % (filtered_obs, minSNR, minSigma, minCts, minExposure)
    )
...
def readsimplePCCURVE(file="PCCURVE.qdp", minExposure=0, minSigma=0, minSNR=0):
    ...
    filtered_data = data[
        (((data["T_+ve"] - data["T_-ve"]) > minExposure) & (data["Sigma"] > minSigma))
    ]
    filtered_obs = len(data) - len(filtered_data)
    print(
        "Filtered %d datapoints by minSNR = %d and minSigma = %.2f"
        % (filtered_obs, minSNR, minSigma)
    )
    return filtered_data


def readPCHR(
    file="PCHR.qdp", minSoftSig=0, minHardSig=0, reject_errors=True, minExposure=0
):
    ...
    try:
        data = np.genfromtxt(
            ...
            skip_header=2,
            comments="!",
            dtype=("f8, f8, f8, f8, f8, f8, f8, f8, f8, f8, f8, f8, f8, U30"),
        )
    except ValueError:
        data = np.genfromtxt(
            "%s" % file,
            ...
            skip_header=2,
            comments="!",
            dtype=("f8, f8, f8, f8, f8, f8, f8, f8, f8, f8, f8, f8, f8"),
        )
    if reject_errors is True:
        filtered_data = data[
            (~np.isnan(data["HR"]))
            & (data["HR"] > 0)
            & (data["SoftSig"] > minSoftSig)
            & (data["HardSig"] > minHardSig)
            & (data["HRerr"] < data["HR"])
            & (data["Exposure"] > minExposure)
        ]
    else:
...
def readPC_catalog(file="PC_catalog.qdp", minExposure=0):
    ...
    data = np.genfromtxt(
        "%s" % file,
        names=["Time", "T_ve", "T_ve_1", "Rate", "Ratepos", "Rateneg"],
        delimiter="\t",
        skip_header=2,
        comments="!",
        dtype=("f8, f8, f8, f8, f8, f8"),
...
def read_best_fit(file):
    ...
    return np.genfromtxt(
        "%s" % file, names=True, delimiter="\t", comments="#", dtype=("f8, f8")
    )
